...
let rec getUndefinedTraverse = (node: t('a), depth: int, code: int, k: k) =>
  switch (node) {
  | Index(bitmap, contents) =>
    ...
    if (has !== 0) {
      ...
      getUndefinedTraverse(child, depth + 1, code, k);
    } else {
      Js.Undefined.empty;
    };
...
let getUndefined = (map: t('v), k: k): Js.Undefined.t('v) => {
  getUndefinedTraverse(map, 0, hash(k), k);
};
...
let rec setOptimisticTraverse = (node, depth, code, k, vbox, optimistic) =>
  switch (node) {
  | Index(bitmap, contents) =>
    let pos = mask(code, depth);
    let has = bitmap land pos;
    ...
    if (has !== 0) {
      let node =
        setOptimisticTraverse(
          Js.Array.unsafe_get(contents, index),
          depth + 1,
        );
      ...
      let contents = Js.Array.copy(contents);
      ignore(
        Js.Array.spliceInPlace(~pos=index, ~remove=0, ~add=[|n|], contents),
      );
      Index(bitmap, contents);
    };
...
let setOptimistic = (map: t('v), k: k, v: 'v, id: int): t('v) => {
  ...
  setOptimisticTraverse(map, 0, hash(k), k, vbox, id !== 0);
};

let set = (map, k, v) => setOptimistic(map, k, v, 0);

let rec removeTraverse = (node, depth, code, k) =>
  switch (node) {
  | Index(bitmap, contents) =>
    let pos = mask(code, depth);
    let has = bitmap land pos;
...
let remove = (map: t('v), k: k): t('v) => {
  removeTraverse(map, 0, hash(k), k);
};

let rec filter = (x: option(boxT('a)), optid) =>
  switch (x) {
...
let clear_box = (box: boxT('a), optid: int) => {
  filter(Some(box), optid);
};

let clearTraverse = (node, depth, optid) =>
  switch (node) {
